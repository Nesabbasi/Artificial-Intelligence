...
class Sim:
    ...
    def _evaluate(self):
        ...
        if self.gameover(self.red, self.blue) == 'red':
            ...
            # redCount = 0
            for child in deepcopy(self.available_moves):
                self.available_moves.remove(child)
                self.red.append(child)
                if self.gameover(self.red, self.blue) == 'red':
                    # redCount += 1
                    count -= 8
                elif self.gameover(self.red, self.blue) == 0:
                    count += 10
                self.available_moves.append(child)
                self.red.remove(child)
            # return redCount
            return count
        elif self.turn == 'blue':
            # blueCount = 0
    ...
    def minimax(self, depth, player_turn):
        ...
        if player_turn == 'red':
            ...
            for child in deepcopy(self.available_moves):
                ...
                self.available_moves.remove(child)
                self.red.append(child)
                move, tempValue =  self.minimax(depth - 1, self.turn)
                self.red.remove(child)
                self.available_moves.append(child)
                self.turn = self._swap_turn(self.turn)
                if tempValue >= value:
